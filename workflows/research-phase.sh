...
# Conductor stays advisory-only under the June/Operator control plane.
if [ "${RESEARCH_USE_CONDUCTOR:-0}" = "1" ] && [ -f "$TOOLS/research_conductor.py" ]; then
  log "Ignoring RESEARCH_USE_CONDUCTOR=1: conductor master mode is disabled under the June/Operator control plane."
fi

# Shadow conductor: log what conductor would decide at this phase (no execution control)
if [ -f "$TOOLS/research_conductor.py" ]; then
