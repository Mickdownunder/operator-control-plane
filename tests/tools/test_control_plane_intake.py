...
def test_submit_ui_research_continue_emits_request_and_dispatch(monkeypatch):
    ...
    def fake_emit(project_id, event_type, payload, event_id=None):
        event = {"event_id": event_id or f"evt-{len(events) + 1}", "project_id": project_id, "event": event_type, **payload}
        events.append(event)
        return event
...
def test_submit_ui_research_start_hands_off_to_june(monkeypatch):
    ...
    assert result["command"] == "ui-research-start"
    assert result["jobId"] == "proj-123"
    assert result["projectId"] == "proj-123"
    assert result["runUntilDone"] is True
    assert handoffs[0][1] == 180
    assert handoffs[0][0][:6] == [
        "ui-research-start",
        "--question",
        "Question?",
        "--research-mode",
    ...
    assert handoffs[0][0][6] == "1"
    assert handoffs[0][0][7:9] == ["--request-event-id", result["requestEventId"]]
    ...
    assert events[0]["event_id"] == result["requestEventId"]
    assert events[-1]["runner"] == "june-control-plane-handoff"
