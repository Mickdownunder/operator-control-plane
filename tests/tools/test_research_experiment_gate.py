...
from tools.research_experiment import _decision_for_run, derive_experiment_gate
...
def test_first_run_is_kept_even_without_metric_improvement():
    assert _decision_for_run(metric_value=0.0, best_value=0.0, has_best_run=False) == 'keep'


def test_non_improving_run_is_discarded_after_best_run_exists():
    assert _decision_for_run(metric_value=0.0, best_value=0.0, has_best_run=True) == 'discard'
    assert _decision_for_run(metric_value=1.0, best_value=1.0, has_best_run=True) == 'discard'


def test_improving_run_is_kept_after_best_run_exists():
    assert _decision_for_run(metric_value=1.0, best_value=0.0, has_best_run=True) == 'keep'
