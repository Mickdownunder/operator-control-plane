...
def test_research_phase_disables_conductor_master_mode_in_control_plane():
    content = (ROOT / "workflows" / "research-phase.sh").read_text(encoding="utf-8")

    assert 'Ignoring RESEARCH_USE_CONDUCTOR=1: conductor master mode is disabled under the June/Operator control plane.' in content
    assert 'python3 "$TOOLS/research_conductor.py" run_cycle "$PROJECT_ID"' not in content
