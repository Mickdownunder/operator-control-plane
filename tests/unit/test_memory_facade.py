...
def test_get_graph_edges_empty(memory_db_path):
    """get_graph_edges with no edges returns []."""
    mem = Memory(db_path=str(memory_db_path))
    rows = mem.get_graph_edges(limit=10)
    mem.close()
    assert rows == []


def test_get_graph_edges_with_filters(memory_db_path):
    """get_graph_edges with from_node_type/to_node_type filters runs without error."""
    mem = Memory(db_path=str(memory_db_path))
    rows = mem.get_graph_edges(from_node_type="strategy_profile", to_node_type="run_episode", limit=5)
    mem.close()
    assert isinstance(rows, list)


def test_state_summary_returns_dict(memory_db_path):
    """state_summary returns dict with expected keys."""
    mem = Memory(db_path=str(memory_db_path))
    summary = mem.state_summary()
    mem.close()
