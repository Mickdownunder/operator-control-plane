...
def test_search_with_query_embedding(memory_conn):
    """search with query_embedding and embedding_json: uses cosine similarity."""
    p = Principles(memory_conn)
    pid = p.insert("guiding", "Use vector similarity", "proj-1", embedding_json="[0.1, 0.2, 0.3]")
    results = p.search("vector", limit=5, query_embedding=[0.1, 0.2, 0.3])
    assert len(results) >= 1
    assert "similarity_score" in results[0]


def test_list_recent_with_domain(memory_conn):
    """list_recent(domain=X) returns only that domain or empty domain."""
    p = Principles(memory_conn)
    p.insert("guiding", "Desc A", "proj-1", domain="oncology")
    p.insert("guiding", "Desc B", "proj-2", domain="")
    out = p.list_recent(limit=10, domain="oncology")
    assert all(r["domain"] == "oncology" or r["domain"] == "" for r in out)
