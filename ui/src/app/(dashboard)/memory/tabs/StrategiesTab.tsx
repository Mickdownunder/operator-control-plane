...
export function StrategiesTab({ strategies, loading }: { strategies: any[] | null; loading: boolean }) {
  ...
  if (loading) {
    return <div className="text-tron-dim p-6">Loading strategy profiles...</div>;
  }
  ...
  return (
    <div className="tron-panel p-6">
      ...
      <p className="text-sm text-tron-dim mb-4">
        Which strategy profiles exist, which are empirical, and how they score by confidence.
      </p>

      {list.length === 0 ? (
        <p className="text-tron-dim">No strategy profiles found.</p>
      ) : (
