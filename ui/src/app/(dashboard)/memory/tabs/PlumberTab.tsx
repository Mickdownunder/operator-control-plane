...
const CAT_META: Record<string, { icon: string; label: string; desc: string }> = {
  shell_syntax:       { icon: "📜", label: "Shell Syntax",     desc: "bash -n syntax check for all workflow scripts" },
  repeated_failures:  { icon: "🔄", label: "Job Failures",     desc: "Repeated workflow failures" },
  python_tools:       { icon: "🐍", label: "Python Tools",     desc: "Compile and import check for all tools" },
  dependencies:       { icon: "📦", label: "Dependencies",     desc: "Installed packages vs. code imports" },
  tool_references:    { icon: "🔗", label: "Tool References",  desc: "Workflow -> tool references" },
  processes:          { icon: "⚙️", label: "Processes",         desc: "Brain processes, zombie processes" },
  venv:               { icon: "🏠", label: "Venv Health",      desc: "Virtual environment and core packages" },
};
...
const GOV_LABELS: Record<number, { label: string; desc: string; color: string }> = {
  0: { label: "Report Only", desc: "Diagnosis only", color: "#64748b" },
  1: { label: "Suggest",     desc: "Diagnosis plus suggestions", color: "#3b82f6" },
  2: { label: "Dry Run",     desc: "Create patches without applying them", color: "#f59e0b" },
  3: { label: "Auto Fix",    desc: "Apply patches automatically", color: "#22c55e" },
};
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "var(--tron-text-muted)" }}>
              7-point diagnosis: shell, Python, dependencies, processes, venv
            </p>
          </div>
        </div>
      ...
      {!report && patches.length === 0 && (
        <div className="text-center py-12" style={{ color: "var(--tron-text-muted)" }}>
          ...
          <p className="text-xs max-w-md mx-auto leading-relaxed">
            Plumber scans the entire system: shell syntax, Python imports,
            dependencies, workflow references, processes, and venv health.
            Issues are detected and automatically repaired at governance level 3.
          </p>
          <button
            onClick={runPlumber}
            disabled={running}
