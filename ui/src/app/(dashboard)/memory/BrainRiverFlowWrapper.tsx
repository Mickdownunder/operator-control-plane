...
export function BrainRiverFlowWrapper({
  ...
  async function startCycle() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        setMessage("Cycle started. Live tracking active.");
        startPolling();
  ...
  return (
    <div className="space-y-4">
          ...
          >
            {cycleRunning ? (
              <span className="flex items-center gap-2">
                <LoadingSpinner className="inline-block w-3 h-3" /> Cycle running...
              </span>
            ) : (
              "Start Brain Cycle"
            )}
      ...
      {showConfirm && (
        <ConfirmDialog
          open
          title="Start brain cycle?"
          message="A new cognitive cycle will start. Phases will appear live in the river visualization."
          confirmLabel="Start"
          cancelLabel="Cancel"
          variant="primary"
