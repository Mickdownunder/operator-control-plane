...
export interface ResearchProjectSummary {
  ...
  experiment_epistemic_status?: string;
  experiment_reason_code?: string;
...
export interface ResearchProjectDetail extends ResearchProjectSummary {
  ...
  /** Autonomous Scientist: experiment-lane result and current lane state. */
  experiment_summary?: {
    experiment_id?: string;
    status?: "running" | "improved" | "inconclusive" | "failed" | "invalid" | string;
    lane_status?: "running" | "candidate_improved" | "improved" | "inconclusive" | "failed" | "invalid" | string;
    epistemic_status?: "unconfirmed" | "confirmed" | "rejected" | string;
    reason_code?: string;
    success: boolean;
    iterations: number;
...
): ResearchProjectDetail["experiment_summary"] {
  if (!lane && !contractResult) return undefined;

  const activeExperimentId = typeof lane?.active_experiment_id === "string" ? lane.active_experiment_id : undefined;
  const briefPath = typeof lane?.brief_path === "string" ? lane.brief_path : undefined;
  const laneStatus = typeof lane?.lane_status === "string" ? lane.lane_status : undefined;
  const epistemicStatus = typeof lane?.epistemic_status === "string" ? lane.epistemic_status : undefined;
  const reasonCode = typeof lane?.reason_code === "string" ? lane.reason_code : undefined;
  const artifactPath = typeof lane?.artifact_path === "string" ? lane.artifact_path : undefined;

  const status = typeof contractResult?.status === "string" ? contractResult.status : undefined;
  const resolvedLaneStatus = typeof contractResult?.lane_status === "string"
    ? contractResult.lane_status
    : laneStatus;
  const resolvedEpistemicStatus = typeof contractResult?.epistemic_status === "string"
    ? contractResult.epistemic_status
    : epistemicStatus;
  const resolvedReasonCode = typeof contractResult?.reason_code === "string"
    ? contractResult.reason_code
    : reasonCode;
  const metricName = typeof contractResult?.metric_name === "string" ? contractResult.metric_name : undefined;
  const metricValue = typeof contractResult?.best_value === "number" ? contractResult.best_value : undefined;
  const resolvedArtifactPath = typeof contractResult?.artifact_path === "string"
    ? contractResult.artifact_path
    : artifactPath;

  return {
    experiment_id: typeof contractResult?.experiment_id === "string" ? contractResult.experiment_id : activeExperimentId,
    status,
    lane_status: resolvedLaneStatus,
    epistemic_status: resolvedEpistemicStatus,
    reason_code: resolvedReasonCode,
    success: status === "improved" || (resolvedLaneStatus === "improved" && resolvedEpistemicStatus === "confirmed"),
    iterations: typeof contractResult?.runs_attempted === "number" ? contractResult.runs_attempted : 0,
    subagents_spawned: 0,
    has_history: false,
    metric_name: metricName,
    metric_value: metricValue,
    terminal_reason: typeof contractResult?.terminal_reason === "string" ? contractResult.terminal_reason : undefined,
    ...
    const projects = await Promise.all(
      projectDirs.map(async (e): Promise<ResearchProjectSummary | null> => {
        ...
        try {
          ...
          const lane = typeof data.experiment_lane === "object" && data.experiment_lane !== null
            ? (data.experiment_lane as Record<string, unknown>)
            : null;
          ...
          return {
            ...
            has_master_dossier,
            experiment_status: typeof lane?.lane_status === "string" ? lane.lane_status : undefined,
            experiment_epistemic_status: typeof lane?.epistemic_status === "string" ? lane.epistemic_status : undefined,
            experiment_reason_code: typeof lane?.reason_code === "string" ? lane.reason_code : undefined,
            active_experiment_id: typeof lane?.active_experiment_id === "string" ? lane.active_experiment_id : undefined,
          };
        } catch {
          return null;
        }
...
export async function getResearchProject(projectId: string): Promise<ResearchProjectDetail | null> {
  ...
  try {
    ...
    try {
      ...
      if (artifactPath) {
        try {
          const resultRaw = await readFile(path.join(projPath, artifactPath, "experiment_result.json"), "utf8");
          const parsed = JSON.parse(resultRaw) as Record<string, unknown>;
          contractResult = parsed && typeof parsed === "object" ? parsed : null;
        } catch {
          // ignore missing contract result while lane is running
        }
      }
      if (!contractResult && activeExperimentId) {
        try {
          const resultRaw = await readFile(path.join(projPath, "experiments", activeExperimentId, "experiment_result.json"), "utf8");
          const parsed = JSON.parse(resultRaw) as Record<string, unknown>;
          contractResult = parsed && typeof parsed === "object" ? parsed : null;
        } catch {
          // ignore missing contract result while lane is running
      ...
      experimentSummary = buildExperimentSummary(lane, contractResult);
    } catch {
      // ignore
    }
    return {
...
export interface ExperimentLaneData {
  brief: Record<string, unknown> | null;
  result: Record<string, unknown> | null;
  trace: ExperimentTraceEntry[];
}

export async function getExperimentLane(projectId: string): Promise<ExperimentLaneData | null> {
  const projPath = safeProjectPath(projectId);
  let project: Record<string, unknown>;
  try {
    project = JSON.parse(await readFile(path.join(projPath, "project.json"), "utf8")) as Record<string, unknown>;
  } catch {
    return null;
  }
  const lane = typeof project.experiment_lane === "object" && project.experiment_lane !== null
    ? (project.experiment_lane as Record<string, unknown>)
    : null;
  const activeExperimentId = typeof lane?.active_experiment_id === "string" ? lane.active_experiment_id : undefined;
  const artifactPath = typeof lane?.artifact_path === "string" ? lane.artifact_path : activeExperimentId ? path.join("experiments", activeExperimentId) : undefined;
  if (!artifactPath) return null;

  let brief: Record<string, unknown> | null = null;
  let result: Record<string, unknown> | null = null;
  let trace: ExperimentTraceEntry[] = [];

  try {
    const raw = await readFile(path.join(projPath, artifactPath, "experiment_brief.json"), "utf8");
    const parsed = JSON.parse(raw) as Record<string, unknown>;
    if (parsed && typeof parsed === "object") brief = parsed;
  } catch {
    // ignore missing brief
  }

  try {
    const raw = await readFile(path.join(projPath, artifactPath, "experiment_result.json"), "utf8");
    const parsed = JSON.parse(raw) as Record<string, unknown>;
    if (parsed && typeof parsed === "object") result = parsed;
  } catch {
    // ignore missing result while running
  }

  try {
    const raw = await readFile(path.join(projPath, artifactPath, "experiment_trace.jsonl"), "utf8");
    trace = raw
      .split("\n")
      .filter(Boolean)
      .map((line) => {
        try {
          return JSON.parse(line) as ExperimentTraceEntry;
        } catch {
          return null;
        }
      })
      .filter((entry): entry is ExperimentTraceEntry => entry !== null)
      .sort((a, b) => (b.iteration ?? 0) - (a.iteration ?? 0));
  } catch {
    // ignore missing trace
  }

  if (!brief && !result && trace.length === 0) return null;
  return { brief, result, trace };
}
