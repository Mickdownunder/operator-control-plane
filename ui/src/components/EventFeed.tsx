...
function humanLabel(type: string, payload: string): { text: string; jobId?: string } {
  if (type === "job") {
    ...
    if (status === "FAILED") {
      return { text: "Failed", jobId };
    }
    if (status === "RUNNING") return { text: "Running...", jobId };
    if (workflowId === "research-init") return { text: "Research project created", jobId };
    if (workflowId === "research-cycle") return { text: "Research run started", jobId };
    if (workflowId?.startsWith("research-")) return { text: `Research: ${status === "DONE" ? "done" : status}`, jobId };
    return { text: status === "DONE" ? "Done" : status ?? payload, jobId };
  }
  return { text: payload };
}
...
export function EventFeed() {
  ...
  if (loading && events.length === 0) return <p className="text-sm text-tron-dim">Loading...</p>;
  if (events.length === 0) return <p className="text-sm text-tron-dim">No events yet.</p>;
  return (
