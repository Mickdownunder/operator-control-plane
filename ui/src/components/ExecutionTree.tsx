...
function getExperimentPhaseStatus(experimentSummary: ExecutionTreeProps["experimentSummary"]): PhaseStatus {
  if (!experimentSummary) return "pending";
  const laneStatus = experimentSummary.lane_status ?? experimentSummary.status;
  const epistemicStatus = experimentSummary.epistemic_status;
  if (laneStatus === "running") return "active";
  if (laneStatus === "candidate_improved") return "active";
  if (laneStatus === "improved" && epistemicStatus === "confirmed") return "done";
  if (laneStatus === "inconclusive") return "done";
  if (laneStatus === "failed" || laneStatus === "invalid") return "failed";
  return experimentSummary.success ? "done" : "pending";
}
...
export function ExecutionTree({
  ...
  const experimentPhaseStatus = getExperimentPhaseStatus(experimentSummary);
  ...
  return (
    <div className="w-full overflow-x-auto pb-1">
      <div className="flex items-start gap-0 min-w-max">
          ...
          return (
            <div key={phase.id} className="flex items-start">
              <div className="flex flex-col items-center gap-1.5" style={{ minWidth: 80 }}>
                <div className={`flex h-8 w-8 items-center justify-center rounded-full border-2 text-[11px] font-bold transition-all ${colors.node}`}>
                  {phaseStatus === "done" ? (
                ...
                <div className="flex flex-col items-center gap-0.5 text-center" style={{ minWidth: 72 }}>
                  <span className={`text-[11px] font-semibold ${colors.text}`}>{phase.label}</span>
                  <span className="text-[9px] font-mono leading-tight" style={{ color: "var(--tron-text-dim)" }}>{phase.desc}</span>
                  ...
                  {phaseStatus === "active" && (
                    <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 rounded" style={{ background: "rgba(68,117,243,0.15)", color: "var(--tron-accent)" }}>
                      ACTIVE
                    </span>
                  )}
                  {phaseStatus === "failed" && (
                    <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 rounded" style={{ background: "rgba(239,68,68,0.15)", color: "rgb(248,113,113)" }}>
                      FAILED
                    </span>
                  )}
                </div>
        ...
        {showExperiment && experimentSummary && (
          <>
            <div className="flex items-center" style={{ marginTop: 15, width: 32 }}>
            ...
            <div className="flex flex-col items-center gap-1.5" style={{ minWidth: 96 }}>
              <div className={`flex h-8 w-8 items-center justify-center rounded-full border-2 text-[11px] font-bold transition-all ${phaseColors[experimentPhaseStatus].node}`}>
              ...
              <span className={`text-[11px] font-semibold ${phaseColors[experimentPhaseStatus].text}`}>Experiment</span>
              <span className="text-[9px] font-mono leading-tight" style={{ color: "var(--tron-text-dim)" }}>Sandbox / Confirm</span>
              {experimentSummary.iterations > 0 && (
                <span className="text-[9px] font-mono" style={{ color: "var(--tron-text-dim)" }}>
                  {experimentSummary.iterations} runs
                </span>
              )}
              {experimentSummary.lane_status && (
                <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: "rgba(68,117,243,0.10)", color: "var(--tron-text-muted)" }}>
                  {experimentSummary.lane_status}
                </span>
              )}
            </div>
