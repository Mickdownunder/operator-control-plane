...
- run: emit a bounded next-action recommendation with state.
- run_cycle: deprecated master mode, allowed only with an explicit manual override.
"""
...
MASTER_MODE_OVERRIDE_ENV = "RESEARCH_CONDUCTOR_ALLOW_MASTER"
...
def run_cycle(project_id: str) -> bool:
    ...
    """
    if os.environ.get(MASTER_MODE_OVERRIDE_ENV, "0").strip() != "1":
        raise RuntimeError(
            "Conductor master mode is disabled under the June/Operator control plane. "
            f"Set {MASTER_MODE_OVERRIDE_ENV}=1 only for explicit manual debugging."
        )
