...
def write_json_atomic(path: Path, data: dict | list, *, backup: bool = True) -> None:
    ...
    if backup and path.exists():
        ...
        if current is not None:
            ...
            backup_tmp = backup_path.with_name(f".{backup_path.name}.tmp.{os.getpid()}")
            backup_tmp.write_text(json.dumps(current, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
...
def _load_json_with_backup(path: Path, default: dict | list | None = None):
    if not path.exists():
...
def load_project(proj_path: Path) -> dict:
    ...
    data = _load_json_with_backup(pj, default={})
    return data if isinstance(data, dict) else {}


def save_project(proj_path: Path, data: dict) -> None:
    write_json_atomic(proj_path / "project.json", data, backup=True)




def load_experiment_lane_result(proj_path: Path) -> dict:
    project = load_project(proj_path)
    lane = project.get("experiment_lane") if isinstance(project.get("experiment_lane"), dict) else {}
    artifact_path = lane.get("artifact_path") if isinstance(lane.get("artifact_path"), str) else None
    experiment_id = lane.get("active_experiment_id") if isinstance(lane.get("active_experiment_id"), str) else None

    candidate_paths = []
    if artifact_path:
        candidate_paths.append(proj_path / artifact_path / "experiment_result.json")
    if experiment_id:
        candidate_paths.append(proj_path / "experiments" / experiment_id / "experiment_result.json")
