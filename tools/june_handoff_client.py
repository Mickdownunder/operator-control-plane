...
def submit_bounded_research_start(
    question: str,
    *,
    source_command: str,
    research_mode: str = "standard",
    ...
    continue_timeout: int = 30,
    max_cycles: int = 1,
) -> dict[str, Any]:
    if max_cycles <= 0:
        raise RuntimeError("max_cycles must be positive")
    ...
    project_id = validate_project_id(str(payload.get("projectId") or ""))
    continue_payload = submit_research_continue(
        project_id,
        source_command=source_command,
    ...
    merged["runUntilDone"] = False
    merged["bounded"] = {
        "maxCycles": max_cycles,
        "continueAccepted": bool(continue_payload.get("ok", True)),
    }
    return merged
