...
Reads the parent report and all child reports + canonical experiment lane results.
...
from tools.june_handoff_client import submit_bounded_research_start
from tools.research_common import llm_call, load_experiment_lane_result, model_for_lane
...
def spawn_followups(parent_id: str, missions: list):
    """Spawn bounded follow-up agents through June-owned control plane."""
    for mission in missions:
        ...
        try:
            payload = submit_bounded_research_start(
                q,
...
def main():
    ...
    parent_exp = load_experiment_lane_result(parent_dir)
    ...
    for d in RESEARCH.glob("proj-*"):
        ...
        if c_json.get("parent_project_id") == parent_id:
            ...
            c_exp = load_experiment_lane_result(d)
...
2. Reports and canonical Sandbox Experiment results from your Field Agents (if any have run yet).
...
- For EACH agent, you must explicitly: (1) state the hypothesis tested, (2) state the outcome (supported / not supported / mixed) with key numbers from the report or experiment lane result, (3) note any limitations or caveats the agent reported. Use exact figures (scores, counts, percentages) where the report or experiment provides them.
