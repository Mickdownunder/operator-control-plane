...
from tools.research_common import load_experiment_lane_result
...
def get_experiment_summary(proj_dir: Path) -> str:
    data = load_experiment_lane_result(proj_dir)
    if not data:
        return ""
    gate = data.get("gate") if isinstance(data.get("gate"), dict) else {}
    parts = [
        f"lane_status={data.get('lane_status')}",
        f"epistemic_status={data.get('epistemic_status')}",
        f"reason_code={data.get('reason_code')}",
        f"objective_met={gate.get('objective_met')}",
    ]
    if isinstance(data.get('best_value'), (int, float)):
        parts.append(f"best_value={data.get('best_value')}")
    if isinstance(data.get('terminal_reason'), str) and data.get('terminal_reason'):
        parts.append(f"terminal_reason={data.get('terminal_reason')}")
    return '; '.join(parts)[:EXPERIMENT_SUMMARY_CHARS]
...
def start_research_question(question: str, dry_run: bool) -> str | None:
    """Start one bounded research step through June-owned control plane."""
    if dry_run:
        print(f"[dry-run] Would start bounded research: {question[:60]}...", file=sys.stderr)
        return None
    try:
        ...
        if not new_project_id:
            return None
        print(f"Started bounded research: {new_project_id} — {question[:50]}...", file=sys.stderr)
